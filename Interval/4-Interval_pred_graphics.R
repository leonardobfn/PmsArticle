...
data.sim.1 = read.table("Interval/pred_sim_method_1.txt")
data.sim.2 = read.table("Interval/pred_sim_method_2.txt")
...
forecast_median_1 = gkumar_pred_quantile(results1,quant = 0.5)
forecast_rr_1 = gkumar_pred_mean(results1)

forecast_median_2 = gkumar_pred_quantile(results2,quant = 0.5)
forecast_rr_2 = gkumar_pred_mean(results2)


dados$forecast_median = c(rep(forecast_median_1[-1],1000),rep(forecast_median_2[-1],1000))
dados$forecast_rr =c(rep(forecast_rr_1$y.pred[-1],1000),rep(forecast_rr_2$y.pred[-1],1000))

DATES = seq(as.Date("2009/2/1"), by = "month", length.out = 131)
...
dados.resultys  =  dados |> mutate(V3 = as.factor(V3), V4 = as.factor(V4)) |>
  ...
  ) |> mutate(Real = data$RH[2:132] |> rep(2),
              Month = DATES |> rep(2),
              forecast_median = c(rep(forecast_median_1[-1],1),rep(forecast_median_2[-1],1)),
              forecast_rr = c(rep(forecast_rr_1$y.pred[-1],1),rep(forecast_rr_2$y.pred[-1],1))
  ) |>
...
dados.resultys |> ggplot(aes(x=Month,y= real_fore)) +
  scale_x_date(date_labels = '%b %y',name = "Date",date_breaks = "1 year") +
  geom_point(size=.2) +
  geom_line(aes(linetype = lines),linewidth=0.2) +
  scale_linetype_manual(values=c("longdash", "dotted","solid"),name="Prediction:",labels=c("Median","Reverse Risk","Real"))+
